...
export function UploadPage() {
  ...
  const busy = items.some(
    item => item.status === 'uploading' || item.status === 'recognizing' || item.status === 'ocr_queued'
  );
    ...
    <div className="page-stack upload-editorial">
      <section className="dashboard-band upload-band editorial-page-heading">
        <div>
          ...
          <h2>
            {uploadMode === 'project_file' ? (
              '把项目资料放到正确的位置。'
            ) : (
              <>
                把原件交给系统，<em>数据从这里开始。</em>
              </>
            )}
          </h2>
          <p>
            {uploadMode === 'project_file'
              ...
              : '上传前拦截不支持的类型以及超过 10MB 和 OCR 尺寸限制的文档。'}
          </p>
        </div>
      ...
      <section className="upload-flow" aria-label="上传识别流程">
        <div className="upload-mode-switch" role="group" aria-label="上传类型">
          <button
            className={uploadMode === 'invoice' ? 'active' : ''}
            disabled={busy}
            onClick={() => changeUploadMode('invoice')}
            type="button">
            发票识别
          </button>
          <button
            className={uploadMode === 'project_file' ? 'active' : ''}
            disabled={busy}
            onClick={() => changeUploadMode('project_file')}
            type="button">
            项目文件
          </button>
        </div>
        <ol className="workflow-steps">
          ...
          <li className={items.length ? 'active' : ''}>
            <span>02</span>确认归属
          </li>
          <li
            className={
              busy ||
              items.some(item => ['uploaded', 'ocr_queued', 'recognizing', 'completed', 'failed'].includes(item.status))
                ? 'active'
                : ''
            }>
            <span>03</span>
            {uploadMode === 'project_file' ? '上传与归档' : '上传与识别'}
          </li>
        </ol>
            ...
            <div className="upload-batch-settings" aria-label="上传设置">
              ...
              {uploadMode === 'invoice' ? (
                <label className="check-row">
                  <input
                    checked={autoOcr}
                    onChange={event => setAutoOcr(event.currentTarget.checked)}
                    type="checkbox"
                  />
                  <span>上传后自动识别</span>
                </label>
              ...
              <button
                ...
                type="button">
                {readyCount ? `上传 ${readyCount} 个文件` : '等待文件校验'}
              </button>
            </div>
          ) : null}
        </div>
